...
from dagpipe.dag import DAGNode, PipelineOrchestrator
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are a Python code generator specializing in DagPipe "
                "pipelines. Write a COMPLETE, RUNNABLE Python script that:\n"
                "1. Reads GROQ_API_KEY from environment\n"
                "2. Creates a Groq client\n"
                "3. Defines LLM wrapper functions (call_groq_8b, call_groq_70b)\n"
                "4. Defines a function for EACH pipeline node with REAL prompts\n"
                "5. Wires ModelRouter with groq 8b as low, 70b as high, "
                "8b as fallback\n"
                "6. Loads the YAML and runs PipelineOrchestrator\n"
                "7. Prints results cleanly\n\n"
                "Each node function must have signature: "
                "def name(context: dict, model=None) -> dict\n"
                "Include inline comments. Use 'from groq import Groq'.\n"
        ...
        },
        {
            ...
            "content": (
                ...
                f"Use case: {use_case}\n"
                f"Nodes:\n{nodes_desc}\n\n"
                f"YAML file will be named 'pipeline.yaml' in the same dir."
            ),
        },
    ]

    # Use the LLM to generate the script
    script_content = model(messages) if model else _fallback_runner(nodes, use_case)
...
def run_generator(
    ...
    orch = PipelineOrchestrator(
        ...
        router=router,
        checkpoint_dir=Path(tempfile.mkdtemp(prefix="dagpipe_ckpt_")),
        max_retries=3,
    )
