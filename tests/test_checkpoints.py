...
def test_checkpoint_saves_file(tmp_path: Path) -> None:
    ...
    checkpoint("node_a", data, directory=tmp_path)
...
def test_restore_loads_correctly(tmp_path: Path) -> None:
    ...
    checkpoint("node_b", original, directory=tmp_path)

    loaded = restore("node_b", directory=tmp_path)
...
def test_restore_returns_none_when_missing(tmp_path: Path) -> None:
    ...
    result = restore("nonexistent_node", directory=tmp_path)
...
def test_checkpoint_exists_returns_true_false(tmp_path: Path) -> None:
    ...
    assert checkpoint_exists("node_c", directory=tmp_path) is False

    checkpoint("node_c", {"status": "done"}, directory=tmp_path)
    assert checkpoint_exists("node_c", directory=tmp_path) is True
...
def test_clear_checkpoints_wipes_directory(tmp_path: Path) -> None:
    ...
    checkpoint("x", {"a": 1}, directory=tmp_path)
    checkpoint("y", {"b": 2}, directory=tmp_path)
    assert len(list(tmp_path.glob("*.json"))) == 2

    clear_checkpoints(directory=tmp_path)
    assert len(list(tmp_path.glob("*.json"))) == 0


def test_list_checkpoints(tmp_path: Path) -> None:
    """list_checkpoints() returns sorted node IDs."""
    checkpoint("beta", {"v": 1}, directory=tmp_path)
    checkpoint("alpha", {"v": 2}, directory=tmp_path)
...
def test_restore_handles_corrupt_json(tmp_path: Path) -> None:
    ...
    result = restore("corrupt", directory=tmp_path)
    assert result is None
